...
# 产品列表和DataFrame初始化
products = ["AU2406.SHF", "AU2412.SHF", "XAUCNY.IDC", "RKGCNH"]
columns = ['Time'] + [f"{prod}_Bid" for prod in products] + [f"{prod}_Ask" for prod in products] + [f"{prod}_Latest" for prod in products if prod != "RKGCNH"]
...
def check_trading_hours(now):
    ...
    if now.weekday() == 5 or now.weekday() == 6:
        return False
    morning_session = (now.time() >= datetime.time(9, 0) and now.time() <= datetime.time(11, 30))
    afternoon_session = (now.time() >= datetime.time(13, 30) and now.time() <= datetime.time(15, 0))
    night_session = (now.time() >= datetime.time(21, 0) or now.time() <= datetime.time(2, 30))
...
def fetch_api_data():
    """从外部API获取数据并更新current_data字典。"""
    url = "https://www.mw801.com/api/screen_price/web_uat.json"
    headers = {
        "Accept": "application/json, text/javascript, */*; q=0.01",
        "Accept-Language": "en-US,en;q=0.5",
        "Referer": "https://www.mw801.com/",
    ...
    try:
        ...
        for item in data['data']:
            if item['name'] == "RKGCNH":
                current_data["RKGCNH"]['Bid'] = float(item['bid'])
                current_data["RKGCNH"]['Ask'] = float(item['ask'])
                break
    except Exception as e:
        print("Failed to fetch or parse API data:", e)
...
def run_wsq():
    ...
    if w.start().ErrorCode != 0:
        ...
        return
    w.wsq("AU2406.SHF,AU2412.SHF,XAUCNY.IDC", "rt_latest,rt_bid1,rt_ask1", func=myCallback)
...
class MarketDataDisplay(QWidget):
    ...
    def initUI(self):
        ...
        self.bid_group.setStyleSheet("QGroupBox { background-color: white; border: 1px solid gray; border-radius: 5px; margin-top: 10px;}")
        self.ask_group.setStyleSheet("QGroupBox { background-color: white; border: 1px solid gray; border-radius: 5px; margin-top: 10px;}")
        self.latest_group.setStyleSheet("QGroupBox { background-color: white; border: 1px solid gray; border-radius: 5px; margin-top: 10px;}")
    ...
    def update_labels(self):
        ...
        if not df.empty:
            ...
            for product in products:
                ...
                text_bid = f'{product}: {bid_price:.2f}' if bid_price is not None else f'{product}: 等待数据...'
                text_ask = f'{product}: {ask_price:.2f}' if ask_price is not None else f'{product}: 等待数据...'
                
                if product == 'AU2406.SHF':
                    self.base_bid = bid_price  # 保存AU2406的Bid价以用于差值计算
                
                if self.base_bid is not None and bid_price is not None and product != 'AU2406.SHF':
                    # 计算与AU2406的差值
                ...
                if product != "RKGCNH":
                    text_latest = f'{product}: {latest_price:.2f}' if latest_price is not None else f'{product}: 等待数据...'
